...
2. Query the comment marker before posting to prevent duplicate posts. The
   marker format is comment format v2's `voc-nexus-comment|v2|<event_id>`
   (issue: `voc-nexus-issue|v2|<event_id>`); see
   ...
   before posting is itself the dedupe lookup. **Note:** the audience-split
   v2 format (see [docs/ARCHITECTURE.md](ARCHITECTURE.md)) is implemented
   as a hard cutover (2026-09-10) — no new event produces a v1 marker. A
   real write adapter's marker lookup must still check both version
   prefixes, because an event processed before the cutover keeps its
   stored `voc-nexus-comment|v1|<event_id>` / `voc-nexus-issue|v1|<event_id>`
   result on replay (per `event_id` + payload fingerprint replay identity,
   not renderer version) rather than assuming every stored event used the
   same format version.
